...
- [ ] T037 [P] Call `purge_expired()` on refresh-token writes (or document scheduled purge) in `backend/app/repositories/refresh_token_repo.py` usage sites — `# ponytail: in-process slowapi limiter; move to Redis when >1 API replica` comment at limiter setup in `backend/app/api/auth.py`
- [ ] T038 [P] Document `/auth/*` + `/admin/jobs` in `specs/001-movie-explorer/contracts/api.md` (or superseding note pointing at `specs/002-user-auth/contracts/`)
- [ ] T039 [P] Document `JWT_SECRET_KEY`, `CORS_ORIGINS`, `ADMIN_EMAIL`/`ADMIN_PASSWORD`, and `python -m app.seed_admin` bootstrap in `README.md`
